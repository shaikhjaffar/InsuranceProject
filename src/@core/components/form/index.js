...
class Form extends Component {
  ...
  handleSubmit() {
    const FNAME_REGEX = new RegExp(/^[a-zA-Z\s]*$/gim)
    const LNAME_REGEX = new RegExp(/^[a-zA-Z\s]*$/gim)
    const PHONE_REGEX = new RegExp(
      /^\s*(?:\+?(\d{1,3}))?[-. (]*(\d{3})[-. )]*(\d{3})[-. ]*(\d{4})(?: *x(\d+))?\s*$/gim
    )
    const EMAIL_REGEX = new RegExp(
      /[a-zA-Z0-9._%+-]+@[a-z0-9.-]+\.[a-zA-Z]{2,4}/gim
    )
    if (this.state.fname === "") {
      ...
      alert(
        "Entered Email-Id is not in correct Format.Email format is example@companyname.com"
      )
      this.setState({ reset: this.state.reset + 1 })
    } else {
      Swal.fire({
      ...
      try {
        ...
        gtag('event', 'conversion', {send_to: 'AW-10846435076/JE6WCKu9nYsYEITu_bMo'})
      } catch (error) {
        alert(error)
      }
    }
      ...
      <div className="form-div">
        ...
        <form>
          <p className="form-msg-p">
            Striving To Get You The Best Financial Solutions To Grow Your
            Business With Ease & Stability.
          </p>
          ...
          <div className="input-container">
            ...
            {this.state.contact === "" ? (
              ...
              <label className="label2">Contact No</label>
            )}
          </div>
          <div className="input-container">
            <input
          ...
          <div className="input-container1">
            <CreatableSelect
              ...
              onChange={this.handlesChange1.bind(this)}
          ...
          <FormGroup check inline style={{ margintop: "-4%" }}>
            <Input
              type="checkbox"
              className="checkbox"
            ...
            />
            <Label check>
              <NavLink onClick={scroll} exact to="/privacy-policy">
                I Agree to{" "}
                <span
          ...
          <h5 className="login-text">
            {" "}
            Already a user <span className="questionmark-form">?</span>{" "}
            <a
              className="link"
              href="https://www.1clickcapital.com/portal/login.php"
            >
              Login
            </a>
          </h5>
